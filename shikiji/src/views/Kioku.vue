  ...
  <div class="container">
    ...
    <div>
      <section class="review-buttons">
        <button @click="next_review_interval(0.1)">Forgot</button>
  ...
  export default {
    ...
    methods: {
      ...
      save_results(result, level) {
        ...
        switch(level) {
          case 0.1:
            this.forgot_counter += 1
            this.review_count = Math.floor(this.review_count / 2)
            the_result = Math.ceil(result / (2 * this.forgot_counter))
            break
          case 1:
            this.hard_counter += 1
            break
          ...
          case 4:
          default:
            this.forgot_counter = 0
        }

        this.review_count += 1
        this.next_review = the_result

        if (level >= 1) this.answer_history.push(level)

        if (this.forgot_counter >= 3) {
          this.forgot_counter = 0
          this.review_count = 0
          for (let i = 0 ; i < 4 ; i++) {
            this.answer_history.push(0)
          }
        }
      }
    },
